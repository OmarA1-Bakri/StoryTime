...
export interface SttProvider {
  transcribe(input: {
    ...
    language?: string;
    timeoutMs: number;
  }): Promise<{ transcript: string; confidence?: number; durationMs?: number }>;
}
...
export interface ImageProvider {
  generateImage(input: {
    prompt: string;
    visualTier: "basic" | "standard" | "premium" | "best";
    timeoutMs: number;
  }): Promise<{
...
export interface SafetyProvider {
  checkText(input: {
    text: string;
    context: "transcript" | "story_beat" | "image_prompt" | "profile_input";
    childAgeBand: string;
