...
describe("story flow", () => {
  it("accepts complete locked story seeds", () => {
    ...
    ).toMatchObject({ character: "Explorer" });
  });
  ...
  it("passes the baton from the current holder", () => {
    expect(
      passBaton(
        { holderParticipantId: "dad", activeSpeakerParticipantId: "dad" },
        "dad",
        "profile",
      ),
    ).toEqual({ holderParticipantId: "profile", activeSpeakerParticipantId: "profile" });
  });
