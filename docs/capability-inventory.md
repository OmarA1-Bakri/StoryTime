...
| Clerk / Convex       | Not configured    | Verified-token bootstrap, signed lifecycle sync, and an additive owner-only family tenant/profile migration foundation exist; no Clerk or Convex account linkage was exercised | Link isolated instances, choose hosted data region, generate/deploy Convex functions, prove identity lifecycle and tenant create/retry/migration/index behavior with synthetic records |
