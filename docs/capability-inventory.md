...
| Clerk                | Not configured    | Web/mobile/Convex adapter foundations and required variable names exist; no configured Clerk variable names were detected in the current environment    | Link dev/preview/prod instances and prove trusted auth-context enforcement                                                                 |
...
| Capability              | Status                                   | Evidence                                                                                                                    | Remaining gate                                                                                                                   |
| ----------------------- | ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| OpenAI story/moderation | Local credential present, policy blocked | An OpenAI credential name is present and the adapters use structured Responses plus moderation; no real child data was sent | Approved zero-retention/no-training configuration, region/terms/deletion review, pinned model/prompt/policy, synthetic benchmark |
| Groq STT                | Not configured                           | Adapter and model default exist; no Groq credential name is configured                                                      | ZDR/private submitted-clip controls, deletion/retention review, credential, quality/latency/cost spike                           |
| fal image               | Not configured                           | Adapter and endpoint variables exist; no fal credential name is configured                                                  | No-store/private output, signed-URL ingestion/ACL/deletion, credential, latency/cost/safety spike                                |
| Full AI-turn spike      | Blocked                                  | OpenAI story/safety can be exercised independently, but bounded audio→STT→story→safety→private image/fallback cannot run    | Close Groq/fal capability gates or approve equivalent providers; use synthetic media only                                        |
