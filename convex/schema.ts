...
export default defineSchema({
  ...
  accessGrants: defineTable({
    ...
    .index("by_adult", ["adultUserId"])
    .index("by_profile_adult", ["profileId", "adultUserId"]),
  ...
  consentRecords: defineTable({
    ...
    method: v.union(
      v.literal("mock"),
      v.literal("payment_card"),
      v.literal("government_id"),
      v.literal("platform_vpc"),
      v.literal("provider_hosted"),
    ),
    provider: v.union(
      v.literal("mock"),
      v.literal("stripe_identity"),
      v.literal("kws"),
      v.literal("other"),
    ),
  ...
  sessions: defineTable({
    ...
    .index("by_profile", ["profileId"])
    .index("by_started_by", ["startedByUserId"])
    .index("by_status", ["status"])
  ...
  presence: defineTable({
    ...
    participantType: v.union(
      v.literal("remote_adult"),
      v.literal("nearby_adult"),
      v.literal("young_participant"),
      v.literal("egress"),
      v.literal("system"),
    ),
    displayName: v.string(),
  ...
  storyChoices: defineTable({
    ...
    category: v.union(
      ...
      v.literal("problem"),
      v.literal("tone"),
      v.literal("visual_format"),
    ),
    options: v.array(v.string()),
    selectedValue: v.optional(v.string()),
    selectedByParticipantId: v.optional(v.string()),
    lockedAt: v.optional(ts),
    createdAt: ts,
  })
    .index("by_session", ["sessionId"])
    .index("by_campaign", ["campaignId"]),
  ...
  turns: defineTable({
    ...
    campaignId: v.id("campaigns"),
    turnIndex: v.number(),
    speakerParticipantId: v.string(),
    speakerRole: v.union(v.literal("remote_adult"), v.literal("young_participant")),
    status: v.string(),
    rawAudioAssetId: v.optional(v.id("assets")),
    transcript: v.optional(v.string()),
    transcriptConfidence: v.optional(v.number()),
    storyBeat: v.optional(v.string()),
    caption: v.optional(v.string()),
    imagePrompt: v.optional(v.string()),
    imageAssetId: v.optional(v.id("assets")),
    nextTurnPrompt: v.optional(v.string()),
    ...
    videoTimestampMs: v.optional(v.number()),
  ...
  recordings: defineTable({
    ...
    ownerUserId: v.id("users"),
    status: v.string(),
    egressIds: v.array(v.string()),
    rawTrackAssetIds: v.array(v.id("assets")),
    finalVideoAssetId: v.optional(v.id("assets")),
    thumbnailAssetId: v.optional(v.id("assets")),
    durationSeconds: v.optional(v.number()),
    storageBytes: v.optional(v.number()),
    startedAt: v.optional(ts),
    stoppedAt: v.optional(ts),
    readyAt: v.optional(ts),
    failureReason: v.optional(v.string()),
    createdAt: ts,
    updatedAt: ts,
  })
  ...
  compositionEvents: defineTable({
    sessionId: v.id("sessions"),
    recordingId: v.optional(v.id("recordings")),
    eventType: v.string(),
    actorParticipantId: v.optional(v.string()),
    ...
    .index("by_session_timestamp", ["sessionId", "timestampMs"]),
  ...
  assets: defineTable({
    ...
    recordingId: v.optional(v.id("recordings")),
    type: v.string(),
    storageProvider: v.union(v.literal("r2"), v.literal("local")),
    bucket: v.string(),
    key: v.string(),
    mimeType: v.string(),
    sizeBytes: v.optional(v.number()),
    checksum: v.optional(v.string()),
    access: v.literal("private"),
    status: v.union(
      v.literal("uploading"),
      v.literal("ready"),
      v.literal("failed"),
  ...
  entitlements: defineTable({
    userId: v.id("users"),
    plan: v.union(v.literal("free"), v.literal("family"), v.literal("pro"), v.literal("vault")),
    storageLimitBytes: v.number(),
    weeklyAdventureCredits: v.number(),
    remainingAdventureCredits: v.number(),
    visualTier: v.union(
      v.literal("basic"),
      v.literal("standard"),
      v.literal("premium"),
    ...
    canCloudBackup: v.boolean(),
    canRemaster: v.boolean(),
    active: v.boolean(),
    provider: v.union(
      v.literal("mock"),
  ...
  safetyEvents: defineTable({
    ...
    action: v.union(v.literal("allowed"), v.literal("modified"), v.literal("blocked")),
    reason: v.string(),
    createdAt: ts,
  }).index("by_session", ["sessionId"]),
  dataRemovalRequests: defineTable({
