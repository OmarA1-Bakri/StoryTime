...
export const addChoiceSet = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("storyChoices", {
...
export const selectChoice = mutation({
  args: {
    choiceId: v.id("storyChoices"),
    selectedValue: v.string(),
    selectedByParticipantId: v.string(),
  },
  handler: async (ctx, args) => {
    const choice = await ctx.db.get(args.choiceId);
    assert(choice, "choice_missing", "Choice set not found");
    assert(
      choice.options.includes(args.selectedValue),
      "choice_invalid",
      "Selected value must be one of the options",
    );
    await ctx.db.patch(args.choiceId, {
      selectedValue: args.selectedValue,
      selectedByParticipantId: args.selectedByParticipantId,
      lockedAt: now(),
    });
  },
});
