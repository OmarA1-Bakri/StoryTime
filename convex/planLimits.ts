...
export const upsertManualPlan = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    assert(
      args.weeklyAdventureCredits >= 0,
      "invalid_weekly_credits",
      "Weekly credits must not be negative",
    );
    assert(
      args.remainingAdventureCredits >= 0,
      "invalid_remaining_credits",
      "Remaining credits must not be negative",
    );
...
export const spendAdventureCredit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(record._id, {
      remainingAdventureCredits: record.remainingAdventureCredits - 1,
      updatedAt: now(),
    });
