...
function request(body: unknown) {
  return new Request("http://localhost/api/livekit/token", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
}
...
describe("LiveKit token route", () => {
  ...
  it("rejects malformed participant data", async () => {
    mockAuth.mockResolvedValue({ userId: "adult-456" } as Awaited<ReturnType<typeof auth>>);
    process.env.LIVEKIT_API_KEY = "key"; process.env.LIVEKIT_API_SECRET = "secret"; process.env.LIVEKIT_URL = "wss://example.livekit.cloud";
  ...
  it("issues a scoped room token", async () => {
    mockAuth.mockResolvedValue({ userId: "adult-456" } as Awaited<ReturnType<typeof auth>>);
    process.env.LIVEKIT_API_KEY = "key"; process.env.LIVEKIT_API_SECRET = "secret"; process.env.LIVEKIT_URL = "wss://example.livekit.cloud";
    const response = await POST(request(validBody)); const body = await response.json();
    expect(response.status).toBe(200); expect(body.serverUrl).toBe("wss://example.livekit.cloud"); expect(body.token.split(".")).toHaveLength(3);
  });
});
